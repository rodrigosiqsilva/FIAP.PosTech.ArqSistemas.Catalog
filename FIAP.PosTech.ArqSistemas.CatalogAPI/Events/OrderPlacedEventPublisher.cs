...
namespace FIAP.PosTech.ArqSistemas.CatalogAPI.Publisher
{
    public record OrderPlacedEventCreated(OrderDto Order, DateTime CreatedAt, string? CorrelationId);

    public class OrderPlacedEventPublisher : IDisposable
    {
        private readonly IProducer<string, string> _producer;
        private readonly string _topicName;

        public OrderPlacedEventPublisher(string bootstrapServers, string topicName)
        {
        ...
        public async Task PublishOrderPlacedEventAsync(OrderPlacedEventCreated orderEvent)
        {
            try
            {
